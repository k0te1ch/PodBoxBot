...
async def uploadToFTP(path: str, filename: str, server: str, login: str,
                      password: str) -> None:
    with ftplib.FTP_TLS(server, login, password, encoding="utf-8") as FTP:
        if "postshow" in filename: # TODO !!!
...
async def checkFileFTP(filename: str, server: str, login: str,
                       password: str) -> bool:
    with ftplib.FTP_TLS(server, login, password, encoding="utf-8") as FTP:
        filelist: list[str] = []
