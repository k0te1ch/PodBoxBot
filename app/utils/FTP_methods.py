...
async def uploadToFTP(path: str, fileName: str, server: str, login: str, password: str) -> None:
...
async def check_file_FTP(fileName: str, server: str, login: str, password: str) -> bool:
    with ftplib.FTP_TLS(server, login, password, encoding="utf-8") as FTP:
        if "postshow" in fileName:
            FTP.cwd("postshow")  #TODO: add to settings
