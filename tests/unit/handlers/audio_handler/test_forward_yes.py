...
from aiogram_tests.types.dataset import USER, MESSAGE, CALLBACK_QUERY, AUDIO
...
async def test_forward_yes(callback_handler_factory, bot_factory, username, language, template_text, expected_call):
    ...
    replied_message = MESSAGE.as_object(message_id=1, from_user=user, text=template_text)
    audio = AUDIO.as_object(file_id="audio_file_id")
    message = MESSAGE.as_object(
        message_id=2, from_user=user, text="Test message", audio=audio, reply_to_message=replied_message
    )
    ...
    with (
        ...
        patch("app.handlers.audio_handler.validate_template", return_value={} if expected_call else None) as mock_validate,
        patch(
            "app.handlers.audio_handler.generate_podcast_text", return_value="Generated podcast text"
        ...
        if expected_call:
            ...
            mock_send_audio.assert_called_once_with(
                chat_id="125135136", audio=audio.file_id, caption="Generated podcast text"
            )
